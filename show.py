...
class setting:
    ...
    def __init__(
        ...
        self.routing = {i: [] for i in range(self.num_nodes + self.lightpole)}
        ...
        while count < num_nodes:
            if count < self.num_nodes:
                ...
                if lightpole_flag:
                    ...
                    pole_queue.put((count, pos))
                    lightpole_flag = False
                    count += 1
                else:
                    current_id, (x, y) = pole_queue.get()
                    base_angles = [0, 60, 120, 180, 240, 300]
                    placed_pole = np.random.choice([1, 2])
                    while placed_pole > 0 and base_angles:
                        chosen_angle = np.random.choice(base_angles)
                        if chosen_angle in base_angles:
                            base_angles.remove(chosen_angle)
                        rad = np.radians(chosen_angle)

                        new_x = x + self.lightpole_distance * np.cos(rad)
                        new_y = y + self.lightpole_distance * np.sin(rad)
                        new_x = round(new_x, 2)
                        new_y = round(new_y, 2)
                        new_pos = (new_x, new_y)
                        ...
                        if new_pos not in [
                            ...
                            if (
                                self.x_range[0] < new_x < self.x_range[1]
                                and self.y_range[0] < new_y < self.y_range[1]
                            ):
                                self.lightpole_path.append((current_id, count))
                                self.positions[count] = new_pos
                                pole_queue.put((count, new_pos))
                                placed_pole -= 1
                                count += 1

        for parent, child in self.lightpole_path:
            self.routing_update(child_node=child, parent_node=parent)

        if self.plot_pattern != 2 and self.plot_pattern != 3:
            self.plot_node()
            for i in self.positions:
                child_node = self.circle_detection(i)
                for count in child_node:
                    self.plot_edge(i, count)
                    self.routing_update(count, i)
                    self.draw()
    ...
    def change_node_color(self, node_ids=[], node_color=""):
        if node_ids is not None:
            ...
            for node_id_1 in self.routing.keys():
                for node_id_2 in self.routing[node_id_1]:
    ...
    def edge_check(self):
        for node_id in self.positions:
            if len(self.routing[node_id]) > 0:
                ...
                for node in self.routing[node_id]:
                    ...
                    if not any(node in child_node):
                        ...
                        # ルーティングテーブルからエッジを削除
                        self.routing[node_id].remove(node)
    ...
    def routing_update(self, child_node=None, parent_node=None):
        ...
        if parent_node is not None and child_node is not None:
            if (
                len(self.routing[parent_node]) < self.node_limits
                and len(self.routing[child_node]) < self.node_limits
            ):
                # 重複の確認
                if (
                    child_node not in self.routing[parent_node]
                    and parent_node not in self.routing[child_node]
                ):
                    self.routing[parent_node].append(child_node)
                    self.routing[child_node].append(parent_node)
                    return True
                else:
                    return False
            else:
    ...
    def search_path(self):
        ...
        while queue:
            ...
            for node_id in lightpole_list:
                new_list.append(node_id)
            child_nodes = new_list

            # 現在のノードが電柱なら、元々繋がっている電柱と繋げる
            if current_node >= self.num_nodes:
                for node_id in self.routing[current_node]:
    ...
    def connection_check(self):
        ...
        parent = {self.start_node: None}  # 親ノードを記録する辞書
        queue = deque([self.start_node])  # 幅優先探索用のキュー
        ...
        while queue:
            ...
            for neighbor in self.routing[current_node]:
